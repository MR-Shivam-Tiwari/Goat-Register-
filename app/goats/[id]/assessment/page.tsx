...
export default async function AssessmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Verify goat exists
  const goatRes = await query("SELECT id_goat FROM goats_data WHERE id_goat = $1", [id]);
  const goat = goatRes.rows[0];
  if (!goat) notFound();
  
  const initialData = await getGoatExpertAssessment(id);
  const cookieStore = await cookies();
  const lang = (cookieStore.get("nxt-lang")?.value as Locale) || (cookieStore.get("NEXT_LOCALE")?.value as Locale) || "ru";
  const t = getTranslation(lang);

  return (
    <div className="min-h-screen bg-[#FAF6EE] py-12 px-6">
       <div className="max-w-xl mx-auto mb-3">
          <Link href={`/goats/${id}`} className="text-blue-600 hover:underline text-sm font-sans">
            {lang === "ru" ? "Назад" : "Back"}
          </Link>
       </div>
