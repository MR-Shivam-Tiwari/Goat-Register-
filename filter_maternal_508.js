...
async function trace() {
  ...
  try {
    ...
    treeRes.rows.forEach(r => {
      // ONLY maternal line: path does not start with "MEF"
      if (r.path.startsWith('MEF')) return;
    ...
    const allLacts = [];
    const reversedPaths = Object.entries(ancestorLacts).reverse();

    reversedPaths.forEach(([path, node]) => {
      node.lactations.forEach((l) => {
        const level = path.length - 2;
        const relationLabel = `Pr: (${level})`;
        allLacts.push({
          ...l,
          relationLabel,
          goatName: node.name,
    ...
    console.log('MATERNAL-ONLY LACTATIONS COUNT:', allLacts.length);
