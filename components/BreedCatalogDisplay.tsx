...
export default function BreedCatalogDisplay({ breeds, t }: BreedCatalogGridProps) {
  ...
  const sortedBreeds = [...(breeds || [])]
    ...
    .sort((a, b) => {
        const cleanName = (name: string) => name.toLowerCase().trim().replace(/ё/g, 'е');
        ...
        const getIndex = (name: string) => {
          ...
          return order.findIndex(o => {
            ...
            return oClean === cleaned || (name === 'Украинская цветная' && oClean === 'украинская пестрая');
          });
        };
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10" suppressHydrationWarning>
      {sortedBreeds.map((breed) => {
        // Rename logic
        const displayName = breed.name === 'Украинская цветная' ? 'Украинская Пёстрая' : breed.name;
        ...
        return (
          ...
          >
            ...
            <div className="aspect-[4/3] w-full overflow-hidden bg-[#D88D33] relative border-b border-gray-100 flex items-center justify-center p-3">
                <img 
                  src={
                    breed.name.toLowerCase().includes('альпийская') ? '/breedimage/Альпийская.png' :
                    breed.name.toLowerCase().includes('зааненская') ? '/breedimage/Зааненская.png' :
                    breed.name.toLowerCase().includes('англо-нубийская') ? '/breedimage/Англо-нубийская.png' :
                    breed.name.toLowerCase().includes('тоггенбургская') ? '/breedimage/Тоггенбургская.png' :
                    breed.name.toLowerCase().includes('карликовая группа') ? '/breedimage/Карликоваягруппакоз.png' :
                    breed.name.toLowerCase().includes('оберхазли') ? '/breedimage/Оберхазли.jpg' :
                    breed.name.toLowerCase().includes('украинская белая') ? '/breedimage/Украинскаябелая.png' :
                    breed.name.toLowerCase().includes('украинская цветная') ? '/breedimage/Украинскаякороткоухая.jpg' :
                    breed.name.toLowerCase().includes('украинская короткоухая') ? '/breedimage/Украинская_короткоухая.jpg' :
                    breed.name.toLowerCase().includes('тюринг') ? '/breedimage/ТюрингскаяЛеснаяКоза.jpg' :
                    breed.name.toLowerCase().includes('бурская') ? '/breedimage/Бурская.jpg' :
                    breed.name.toLowerCase().includes('ла-манча') ? '/breedimage/ЛаМанча.jpg' :
