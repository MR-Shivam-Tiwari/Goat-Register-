...
import { Upload, X, ChevronLeft, Save, Image as ImageIcon, Check } from 'lucide-react';
import { addBreedAction } from '@/lib/actions';

export default function BreedAddForm({ t }: { t: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        data.append('icoFile', selectedFile);
      }

      const result = await addBreedAction(data);

      if (result.success) {
        setMessage({ type: 'success', text: 'Breed added successfully!' });
        setTimeout(() => {
          router.push('/catalog/breeds');
          router.refresh();
        }, 1500);
      } else {
        setMessage({ type: 'error', text: result.error || 'Failed to add breed' });
      }
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gray-100 pb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {loading ? (t.common.saving || 'SAVING...') : m.saveBreed}
          </button>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-8 border border-gray-100 rounded-sm shadow-sm">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={e => setFormData({ ...formData, id_family: parseInt(e.target.value) || 1 })}
                  className="w-full bg-[#F9F9F9] border-b-2 border-transparent focus:border-[#491907] p-4 text-sm font-bold transition-all outline-none"
                />
              </div>
              <div className="space-y-1.5">
                ...
                {!selectedFile ? (
                  <div className="relative group">
                    <input
                      ...
                      accept="image/*"
                    />
                    <div className="bg-white border-2 border-dashed border-gray-300 p-12 text-center rounded-sm group-hover:border-[#491907] transition-all">
                      <Upload className="mx-auto text-gray-400 group-hover:text-[#491907] mb-2" size={48} />
