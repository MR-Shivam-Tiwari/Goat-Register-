...
export default function Breadcrumbs({ items, isGuest, t }: { items: BreadcrumbItem[], isGuest?: boolean, t?: any }) {
  ...
  const registries = [
    { label: t?.rules?.rhbTitle?.toUpperCase() || "MAIN REGISTRY (RHB)", href: `/catalog/goats/${alias}/${sex}?reg=rhb` },
    { label: t?.rules?.rcbTitle?.toUpperCase() || "ABSORPTION CROSSING REGISTRY (RCB)", href: `/catalog/goats/${alias}/${sex}?view=rcb` },
  ...
  return (
    <div className="w-fit bg-[#D9C1A3] flex px-6 py-2.5 rounded shadow-sm border border-[#CFA979]/50 select-none">
      <nav ref={navRef} className="flex items-center flex-wrap gap-x-3 gap-y-1 text-[12px] font-bold text-[#491907]">
      {!isGuest && (
        <span className="opacity-60 normal-case font-medium mr-1 whitespace-nowrap">{t?.common?.youAreHere || 'You are here:'}</span>
      )}
      
      {/* HOME */}
      <div className="flex items-center gap-3">
        <Link href="/" target="_blank" className="hover:underline transition-colors whitespace-nowrap">
          {t?.nav?.home || 'Home'}
        </Link>
      ...
      {/* CATALOG */}
      <div className="flex items-center gap-3">
        <Link href="/catalog/goats" target="_blank" className="hover:underline transition-colors whitespace-nowrap">
      ...
      {items.filter(item => {
        ...
        if (type === 'breed' || (index === 0 && !isGuest && !item.label.toLowerCase().includes('catalog') && !item.label.toLowerCase().includes('каталог'))) {
            const breedKey = Object.keys(breedNames.en).find(k => 
              breedNames.en[k].toLowerCase() === item.label.toLowerCase() || 
              breedNames.ru[k].toLowerCase() === item.label.toLowerCase() || 
              (item.label.toLowerCase().includes('пестрая') && breedNames.ru[k].toLowerCase().includes('цветная')) ||
              (item.label.toLowerCase().includes('цветная') && breedNames.ru[k].toLowerCase().includes('пестрая')) ||
              k === alias
            );
            if (breedKey) {
        ...
        return (
          ...
          >
            ...
            >
                <span className={`tracking-tight ${isLast ? "font-black" : ""}`}>
                    {labelToDisplay}
                </span>
            ...
            {isOpen && dropdownItems.length > 0 && (
              <>
                ...
                <div 
                  className="absolute top-[calc(100%+5px)] left-0 min-w-[280px] bg-white border border-gray-200 shadow-2xl z-[9999] py-1 rounded-sm overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200"
                >
            ...
            {!isLast && <span className="text-[#491907]/30 font-light select-none">•</span>}
          </div>
        );
      })}
    </nav>
