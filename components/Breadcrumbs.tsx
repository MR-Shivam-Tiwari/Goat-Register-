...
export default function Breadcrumbs({ items, isGuest, t }: { items: BreadcrumbItem[], isGuest?: boolean, t?: any }) {
  return (
    <div className="modern-breadcrumb flex items-center gap-2 text-[10px] mb-6 py-2 px-4 bg-[#491907]/5 rounded-lg w-fit uppercase font-bold tracking-widest">
      {!isGuest && (
        <>
          <Link href="/" className="text-[#491907] hover:text-[#CFA979] transition-colors flex items-center gap-1.5 opacity-60 hover:opacity-100 uppercase">
      ...
      {items.map((item, index) => (
        <div key={index} className="flex items-center gap-2">
          {index > 0 && <span className="text-gray-300 font-light select-none text-[8px]">/</span>}
          {item.href ? (
